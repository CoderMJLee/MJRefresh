...
static BOOL _respondsAdjustedContentInsetMethod;
- (BOOL)isRespondsAdjustedContentInset
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _respondsAdjustedContentInsetMethod = ([[[UIDevice currentDevice] systemVersion] compare:@"11" options:NSNumericSearch] != NSOrderedAscending) && [self respondsToSelector:@selector(adjustedContentInset)];
    });
    return _respondsAdjustedContentInsetMethod;
}

- (UIEdgeInsets)mj_inset
{
#ifdef __IPHONE_11_0
    if ([self isRespondsAdjustedContentInset]) {
